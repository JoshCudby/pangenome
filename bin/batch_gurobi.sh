...
printf "\n\n"
echo "Gurobi Solver"
bsub -J  "gurobiJobs[1-$jobs]" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -M "$memory" -o "out/gurobi.$filename.%J.%I" -e "out/error.gurobi.$filename.%J" -G "qpg" "python3 ./tangle/max_path_gurobi.py $filename $normalisation $time_limit"
