...
do
    ...
    bsub -m "node-5-10-4" -J  "o_t_mqlib[1-$jobs]" -R '"select[mem>'$memory'] rusage[mem='$memory']"' -M "$memory" -o "out/oriented/mqlib.full.$filename.%J.%I" -e "out/oriented/error.mqlib.full.$filename.%J" -G "qpg" "python3 ./oriented_tangle/oriented_max_path_mqlib.py $filename $normalisation $time_limit"
done
